        ...
        try {
         ...
         const reSize = async(buffer, ukur1, ukur2) => {
             ...
             resolve(ab)
                ...
                let butwel = [{ buttonId: 'idk', buttonText: { displayText: 'ようこそ' }, type: 1 }]
                let butleav = [{ buttonId: 'idk', buttonText: { displayText: 'いらっしゃいませ' }, type: 1 }]
                let butselamat = [{ buttonId: '', buttonText: { displayText: '幸せ' }, type: 1 }]
                let butsebar = [{ buttonId: '', buttonText: { displayText: '我慢しろ兄貴' }, type: 1 }]
                let nyoutube = ('Slime Bot')
                let teks1 = `*Halo Kak @${num.split('@')[0]}*\n*Selamat Datang Di Grup*\n*${metadata.subject}*\n*Jangan Lupa Intro Yahh*\n_~Admin_`
                let teks2 = `*Selamat Tinggal Kak @${num.split('@')[0]}*\n*Semoga tenang di isekai sana*\n_~Admin_`
                let teks3 = `*@${num.split('@')[0]} Promote From*\n*${metadata.subject}*\n*Selamat Anda Menjadi Admin*\n_Kemren bang!_`
                let teks4 = `*@${num.split('@')[0]} Demote From*\n*${metadata.subject}*\n_Yahh turun pangkat_`
                if (anu.action == 'add') {
